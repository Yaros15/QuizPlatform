...
namespace QuizPlatform.DTO.Requests
{
    ...
    // Запрос на прохождение квиза
    public class SubmitQuizRequest
    {
        [Required(ErrorMessage = "Ответы обязательны")]
        [MinLength(1, ErrorMessage = "Должен быть хотя бы 1 ответ")]
        public List<AnswerSubmissionRequest> Answers { get; set; } = new();
    }

    // Отдельный ответ на вопрос
    public class AnswerSubmissionRequest
    {
        [Required(ErrorMessage = "ID вопроса обязателен")]
        public long PuzzleId { get; set; }

        [Required(ErrorMessage = "ID выбранного ответа обязателен")]
